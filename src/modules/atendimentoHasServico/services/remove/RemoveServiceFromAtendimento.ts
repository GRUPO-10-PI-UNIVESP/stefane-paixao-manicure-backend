...
export default class RemoveServiceFromAtendimento
{
    ...
    async execute(servicoId: number, atendimentoId: number): Promise<void>
    {
        ...
        const atendimentoHasServico: IAtendimentoHasServico = await new GetAtendimentoHasServico(new AtendimentoHasServicoRepository()).execute(servicoId, atendimentoId);
